 ...
 * AABB may seem like an ugly name, but it is an actual term 
 * used in literature.
 * <p>
 * I guess this class is empty, right now.  Eventually, I will put 
 * stuff in here.  Aabb arrays are meant to be a fast, mutable 
 * alternative to Manifold.Cube objects, which are misnamed
 * anyway.
 * 
 * @author decamp
 */
public final class Aabbs {
    ...
    public static Aabb arrayToAabb(double[] aabb) {
        return Aabb.fromEdges(aabb[0], aabb[1], aabb[2], aabb[3], aabb[4], aabb[5]);
    }
    ...
    /**
     * Haha!  A clipping method that works!  Provided with an array of vertices (assumed to be coplanar), and
     * an AABB, this method will give you the geometric intersection.
     * 
     ...
     * @return true iff intersection is intersection is found with non zero surface area. 
     * (Sorry; this statement isn' precisely true because I didn't formalize all the boundary conditions)
     */
    ...
    private static int removeLoopSection(double[][] v, int count, int start, int stop, int axis, double pos) {
        ...
        //Make sure there's enough room between start and stop vertices if they're adjacent.
        if(start + 1 == stop) {
